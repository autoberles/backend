...
public class WheelDriveTypeTests
{
    ...
    public async Task ReturnsAllWheelDriveTypes()
    {
        ...
        var actionResult = await controller.GetAllWheelDriveTypes();
        var okResult = actionResult as OkObjectResult;
        if (okResult == null)
            throw new Exception("No OkObjectResult was received from the API.");
        var data = okResult.Value as List<WheelDriveType>;
        if (data == null)
    ...
    public async Task ReturnsWheelDriveTypeById()
    {
        var context = TestDbContextFactory.Create();
        var controller = new WheelDriveTypeController(context);
        var actionResult = await controller.GetWheelDriveTypeById(1);
        var okResult = actionResult as OkObjectResult;
        if (okResult == null)
            throw new Exception("No OkObjectResult was received from the API.");
        var data = okResult.Value as WheelDriveType;
        if (data == null)
